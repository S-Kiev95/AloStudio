...
| 6 | Automation + Macros + Labels + CSAT | ✅ done       | Label CRUD + rename cascade, Macro CRUD + visibility + execute, AutomationRule CRUD + condition evaluator + ActionExecutor + dispatcher-event listener with loop prevention, CSAT send-on-resolve + public submit + dashboard metrics. Real wire-parity fix: ``current_user`` 401 now emits devise's canonical ``{"errors":["You need to sign in or sign up before continuing."]}`` envelope unwrapped. CSV download + WhatsApp/Twilio CSAT templates deferred to phase 7-9 |
| 7 | Reports                         | ✅ done       | ReportingEvent listener (resolve / first-response / reply_time / reopen); /reports/summary (cards + previous window); /reports?metric (timeseries: 9 metrics, daily buckets, tz-aware); /reports/conversations (live counters); /live_reports (account + grouped by team/assignee); /summary_reports (agent/team/inbox/label). CSV renderers + bot_summary + bot_metrics + year_in_review + conversation_traffic + inbox_label_matrix + first_response_time_distribution + outgoing_messages_count + channel summary deferred to phase 7 follow-up / phase 8 / phase 10 |
| 8 | Integrations (Slack, Dialogflow, Webhook apps, OpenAI) | ✅ done       | AgentBot CRUD + inbox attach/detach + dispatcher listener (HMAC-signed POST to outgoing_url); Webhook CRUD + subscription-based delivery listener; Integration hooks scaffold + static apps catalogue (slack/dialogflow/webhook/openai/linear/shopify/notion/dyte). Per-vendor adapters (Slack OAuth, Dialogflow routing, OpenAI reply assist, Shopify orders, Linear issues, Dyte rooms) + process_event runtime deferred to vendor-specific follow-up phases. Captain (AI inference events) + access_token encryption deferred to phase 10 hardening |
| 9 | Admin advanced (audit log, working hours, SLA, portal, campaigns) | ✅ done       | Working hours (per-inbox 7-row schedule + bulk update + business-hours math wired into Phase 7's ``value_in_business_hours``); Help Center CRUD (Portal/Category/Article); Campaigns CRUD (ongoing + one_off, display_id sequential, completed-immutable). SLA + Audit log read endpoint are enterprise-only in Chatwoot OSS — **deferred** with schema slots reserved for Phase 10. Public Help Center surface + campaign scheduler runtime defer to Phase 10 |
| 10 | Hardening (perf, observability, real-DB import) | ✅ done       | ARQ worker + 5-min scheduler (one_off campaign fire, snoozed-conversation reopen); public Help Center surface (``/hc/<slug>``); RequestIdMiddleware (correlation id + structlog binding); ``/health`` with DB+Redis ping + uptime + version; enterprise-only schema sweep (audits + sla_policies + applied_slas tables) so pg_dump→pg_restore from a Chatwoot reference works without drift. Per-channel campaign send pipeline + Sentry/OTLP exporter wiring + access_token encryption stay as deployment-specific follow-ups |
