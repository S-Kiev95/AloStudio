...
Note there is deliberately **no** public/unauthenticated door here: outbound
media goes to Meta by uploading the bytes to ``/me/message_attachments``
(see :mod:`app.domains.instagram.sender`), so nothing external ever needs to
pull a blob from us.
"""
...
async def _stream_attachment(att: Attachment) -> Response:
    ...
    when it isn't from the browser) and hand the bytes back."""
